...
const Results = (props) => 
{
	...
	const numFlights = props.offers.length >= 15 ? 15 : props.offers.length;
	...
	return (
		<div>
			{((outboundTrips.length === 0) && (inboundTrips.length === 0)) ? "No flights found." : (
				props.tripType === 'roundTrip' ? (
					outboundTrips.map(trip => (
						<TwoWayItinerary key={trip.id} outbound={trip} inbound={inboundTrips[trip.id]} price={trip.price} />
					))
				) : (
					outboundTrips.map(trip => (
						<OneWayItinerary key={trip.id} outbound={trip} price={trip.price} />
					))
				)
			)}
		</div>
